...
export default function PropertyCard({ property, viewMode = "grid" }: PropertyCardProps) {
  ...
  return (
    <Link href={`/properties/${property.id}`} className="block touch-manipulation">
      <div className="md:hidden p-[2px] rounded-2xl bg-gradient-to-r from-purple-500 via-orange-400 to-purple-600 shadow-[0_0_15px_rgba(139,92,246,0.4),0_0_30px_rgba(251,146,60,0.2)] animate-pulse-slow">
      <Card className="bg-white dark:bg-slate-800 rounded-2xl shadow-lg hover:shadow-xl transition-all duration-300 overflow-hidden group h-full md:shadow-lg">
      ...
      <CardContent className="p-4">
        ...
        {/* View Details Button */}
        <Button className="w-full bg-gradient-to-r from-purple-600 to-orange-500 hover:from-purple-700 hover:to-orange-600 text-white touch-manipulation">
          View Property Details
        </Button>
      </CardContent>
    </Card>
      </div>
      {/* Desktop version without glow wrapper */}
      <Card className="hidden md:block bg-white dark:bg-slate-800 rounded-2xl shadow-lg hover:shadow-xl transition-all duration-300 overflow-hidden group h-full">
        <div className="relative overflow-hidden">
          <img
            src={getImageSrc()}
            alt={property.title}
          ...
          <div className="absolute top-3 left-3 flex flex-col gap-2">
            <div className="flex gap-2">
              <Badge className={`text-white px-2 py-1 text-xs rounded-md ${property.listingType === 'rent' ? 'bg-blue-600' : 'bg-green-600'}`}>
                {property.listingType === 'rent' ? 'Rental' : 'Sale'}
              </Badge>
              {property.featured && (
                <Badge className="bg-orange-primary text-white px-2 py-1 text-xs rounded-md">Featured</Badge>
        ...
        <CardContent className="p-4">
          <h3 className="text-lg font-semibold text-slate-800 dark:text-white mb-2 line-clamp-1">{property.title}</h3>
          <p className="text-slate-600 dark:text-slate-300 mb-3 flex items-center text-sm">
            <MapPin className="w-4 h-4 mr-1" />
            {property.address}, {property.suburb}, {property.city}
          </p>
          ...
          {property.features && property.features.length > 0 && (
            <div className="flex gap-2 mb-4 flex-wrap">
              {property.features.map((feature, index) => {
                ...
                return (
                  <span key={index} className={`inline-flex items-center px-2.5 py-1 rounded-full text-xs font-medium ${colors[index % colors.length]}`}>
                    {feature}
                  </span>
                );
              })}
