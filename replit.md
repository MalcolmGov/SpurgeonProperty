...
- **June 21, 2025 - Homepage Live Search Implementation**: Implemented live search functionality on homepage PropertySearch component. Added real-time filtering that updates featured properties as users type, supporting search across title, address, suburb, and city fields. Homepage now shows filtered results immediately instead of redirecting to Properties page. Enhanced PropertySearch with onSearchChange prop for dual functionality - live search on homepage and navigation on other pages. Updated section titles to show "Search Results for [term]" when filtering active.
